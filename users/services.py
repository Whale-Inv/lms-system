...
class StripeService:
    ...
    def create_product(self, name, description=None, metadata=None):
        ...
        response = requests.post(
            f"{self.base_url}/products", headers=self.headers, data=data
        )
    ...
    def create_price(self, product_id, unit_amount, currency="usd"):
        ...
        response = requests.post(
            f"{self.base_url}/prices", headers=self.headers, data=data
        )
        return response.json()

    def create_checkout_session(
        self, price_id, success_url, cancel_url, quantity=1, metadata=None
    ):
        """
        ...
        response = requests.post(
            f"{self.base_url}/checkout/sessions", headers=self.headers, data=data
        )
        return response.json()
    ...
    def retrieve_checkout_session(self, session_id):
        ...
        response = requests.get(
            f"{self.base_url}/checkout/sessions/{session_id}", headers=self.headers
        )
        return response.json()
    ...
    def create_full_payment_flow(
        ...
        price = self.create_price(
            product_id=product["id"], unit_amount=unit_amount, currency=currency
        )
